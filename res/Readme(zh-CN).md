# 写在开头
 ...
 __如果您需要正體字版本的使用說明請按[此處](https://github.com/hanaonnao/liverecord)__
...
此处列举自动录播脚本运行所需的所有程序，如果一键脚本安装失败或希望手动安装环境可以参考  
  ...
  * [ffmpeg](https://github.com/FFmpeg/FFmpeg)，安装方法为`sudo apt install ffmpeg`。否则无法使用除了youtube、twitcast、twitcastpy、nicolv、nicoco、nicoch、bilibili、bilibiliproxy以外的参数。如果需要最新的版本可以使用一键脚本进行安装，亦或是[参考此处](https://www.johnvansickle.com/ffmpeg/faq/)。
...
__请注意，请您务必在record.sh目录下运行record.sh。即便您使用后台运行也请您在目录下运行。  
否则会出现找不到文件的情况__  
...
__请请注意，请您务必在record.sh目录下运行record.sh。  
如果您在别的目录下运行record.sh的话将会直接出现未登入的情况，即便您已经设置好Cookies.txt__  
如果已经生效：
```

...
while true; do
		...
		if [[ "${1}" == "bilibili"* ]]; then
			if (wget -q -O- "https://api.live.bilibili.com/room/v1/Room/get_info?room_id=${PART_URL}" | grep -q '"live_status":1'); then
				if [[ "${EXCEPT_YOUTUBE_PART_URL}" != "noexcept" ]]; then
					...
					(wget -q -O- "${EXCEPT_YOUTUBE_FULL_URL}" | grep -q '\\"qualityLabel\\":\\"[0-9]*p\\"') && echo "${LOG_PREFIX} restream from ${EXCEPT_YOUTUBE_FULL_URL} retry after ${TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
				fi
				if [[ "${EXCEPT_TWITCAST_PART_URL}" != "noexcept" ]]; then
					LOG_PREFIX=$(date +"[%Y-%m-%d %H:%M:%S]") ; echo "${LOG_PREFIX} metadata ${EXCEPT_TWITCAST_FULL_URL}"
					(wget -q -O- "https://twitcasting.tv/streamserver.php?target=${EXCEPT_TWITCAST_PART_URL}&mode=client" | grep -q '"live":true') && echo "${LOG_PREFIX} restream from ${EXCEPT_TWITCAST_FULL_URL} retry after $TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
				fi
				if [[ "${EXCEPT_TWITCH_PART_URL}" != "noexcept" ]]; then
					...
					(echo "${EXCEPT_TWITCH_STREAM_URL}" | grep -q ".m3u8") && echo "${LOG_PREFIX} restream from ${EXCEPT_TWITCH_FULL_URL} retry after ${TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
				fi
				if [[ "${EXCEPT_OPENREC_PART_URL}" != "noexcept" ]]; then
					...
					[[ -n "${EXCEPT_OPENREC_LIVE_URL}" ]] && echo "${LOG_PREFIX} restream from ${EXCEPT_OPENREC_FULL_URL} retry after ${TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
				fi
				...
				if [[ "${EXCEPT_NICOLV_PART_URL}" != "noexcept" ]]; then
					...
					[[ -n "${EXCEPT_NICOLV_LIVE_URL}" ]] && echo "${LOG_PREFIX} restream from ${EXCEPT_NICOLV_FULL_URL} retry after ${TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
				fi
				if [[ "${EXCEPT_NICOCO_PART_URL}" != "noexcept" ]]; then
					...
					[[ -n "${EXCEPT_NICOCO_LIVE_URL}" ]] && echo "${LOG_PREFIX} restream from ${EXCEPT_NICOCO_FULL_URL} retry after ${TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
				fi
				if [[ "${EXCEPT_NICOCH_PART_URL}" != "noexcept" ]]; then
					...
					[[ -n "${EXCEPT_NICOCH_LIVE_URL}" ]] && echo "${LOG_PREFIX} restream from ${EXCEPT_NICOCH_FULL_URL} retry after ${TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
					...
					[[ -n "${EXCEPT_NICOCH_LIVE_ID}" ]] && echo "${LOG_PREFIX} restream from ${EXCEPT_NICOCH_FULL_URL} retry after ${TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
				fi
				...
				if [[ "${EXCEPT_MIRRATIV_PART_URL}" != "noexcept" ]]; then
					...
					[[ -n "${EXCEPT_MIRRATIV_LIVE_URL}" ]] && echo "${LOG_PREFIX} restream from ${EXCEPT_MIRRATIV_FULL_URL} retry after ${TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
				fi
				if [[ "${EXCEPT_REALITY_PART_URL}" != "noexcept" ]]; then
					...
					[[ -n "${EXCEPT_REALITY_STREAM_URL}" ]] && echo "${LOG_PREFIX} restream from ${EXCEPT_REALITY_FULL_URL} retry after ${TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
				fi
				...
				if [[ "${EXCEPT_CHATURBATE_PART_URL}" != "noexcept" ]]; then
					...
					[[ -n "${EXCEPT_CHATURBATE_LIVE_URL}" ]] && echo "${LOG_PREFIX} restream from ${EXCEPT_CHATURBATE_FULL_URL} retry after ${TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
				fi
				if [[ "${EXCEPT_STREAM_PART_URL}" != "noexcept" ]]; then
					...
					(echo "${EXCEPT_STREAM_STREAM_URL}" | grep -Eq ".m3u8|.flv|rtmp:|The stream specified cannot be translated to a URL") && echo "${LOG_PREFIX} restream from ${EXCEPT_STREAM_FULL_URL}. retry after ${TURELOOPINTERVAL} seconds..." && sleep ${TURELOOPINTERVAL} && continue
				fi
	...
	if [[ "${1}" == "17live" ]]; then
		(ffmpeg -headers "Referer: https://17.live/live/${PART_URL}" -i "${STREAM_URL}" -codec copy -f mpegts "${DIR_LOCAL}/${FNAME}" > "${DIR_LOCAL}/${FNAME}.log" 2>&1) &
	fi
	...
	if [[ ! -f "${DIR_LOCAL}/${FNAME}" ]] || [[ $(ls -l "${DIR_LOCAL}/${FNAME}" | awk '{print $5}') == 0 ]]; then #判断是否无录像
		...
		if [[ "${BACKUP_DISK}" == *"baidupan"* ]]; then			
			until [[ ${BAIDUPAN_FILE_RETRY} -gt ${BACKUP_RETRY_MAX} ]]; do
				...
				BAIDUPAN_FILE_ERRFLAG=$(BaiduPCS-Go upload -nosplit "${DIR_LOCAL}/${FNAME}" "${DIR_BAIDUPAN}")
		...
		if [[ "${BACKUP_DISK}" == *"baidupan"* ]]; then
			until [[ ${BAIDUPAN_LOG_RETRY} -gt ${BACKUP_RETRY_MAX} ]]; do
				...
				BAIDUPAN_LOG_ERRFLAG=$(BaiduPCS-Go upload -nosplit "${DIR_LOCAL}/${FNAME}.log" "${DIR_BAIDUPAN}")
